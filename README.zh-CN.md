...
Web 界面做主题：配色、吉祥物徽章、背景、注入语气都能自己调。背景同时支持**静态图片与动态 GIF**
（上传动画 GIF 会自动识别为动态背景）。默认是一套魈的青玉/翠青风格，但主色、徽章文字、语气、
背景等都可配置，改出来就是你的专属主题。
...
- **吉祥物徽章**：可拖动、可收起的小徽章（底部右侧），标题 / 副标可改成任意文字，头像图也可直接上传替换。头像同样支持 GIF 动图，上传 GIF 即以动图播放。
- **魈式语气**：向系统提示注入一段魈式语气说明（可开关），语言可选中文 / 英文模板，也可填自定义提示词。
- **磨砂背景**：可配置背景图（支持相对插件目录或本地绝对路径，也可直接上传），并调节模糊强度与界面的不透明度。上传 **GIF 动图** 会自动识别并作为 **动态背景**（动画铺满）；静态图片或单帧 GIF 仍按静态磨砂背景处理。
...
- **设置页**：总开关、主题颜色、注入语气、模板语言、自定义提示词、头像路径、吉祥物标题/副标、主题管理、磨砂背景（开关/路径/上传/模糊/透明度，GIF 自动识别为动态背景）、界面与侧栏不透明度。
...
- 打开 DSH Web → **设置 → 魈主题** 页：总开关、主题颜色、注入语气、模板语言、自定义提示词、头像路径、
  吉祥物标题/副标、主题管理、磨砂背景（开关 / 路径 / 上传 / 模糊 / 透明度，GIF 自动识别为动态背景）、界面不透明度、侧栏不透明度。
